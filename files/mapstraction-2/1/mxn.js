	...
	return {
		...
		addProxyMethods: function(func, aryMethods, bWithApiArg){
			for(var i = 0; i < aryMethods.length; i++) {
				...
				func.prototype[sMethodName] = function(){
					return this.invoker.go(sMethodName, arguments);
				};
			}
		},
		...
		*/
		...
		Invoker: function(obj, sClassName, fnApiIdGetter){
			this.go = function(sMethodName, args){
				var sApiId = fnApiIdGetter.apply(obj);
				if(typeof(sApiId) == 'string'){
					if(arguments.length > 2){
						// make sure args is an array
						args = Array.slice.apply(args);
						var extras = Array.slice.call(arguments, 2);
						args = args.concat(extras);
					}
					mxn.invoke(sApiId, sClassName, sMethodName, obj, args);
				}
				else{
